...
Inheiriting from the nn.Module base class, the `NeuralODE` class creates the foundational outline of a neural network by defining the derivative funtion $\frac{d \mathbf{h}(t)}{dt} = f(\mathbf{h}(t), t, \theta)$ that follows a general feed foreward architecture to generate the derivative for each state $h(t)$ over a continuous time interval. The `ODE` class implemenets the numerical integral solver that computes for the predicted values at each state by calling the `NeuralODE` class and implementing its calculated derivates. The Dormand-Prince-5 (DOPRI-5) numerical method is a Runge Kutta method of order 8 that is widely used for integrating ODEs where high precision is needed and adaptive time steps are used.
...
The final optimized `CNN_ODE` class consists of 36 kernels each 3 ints long that slides along the data one at a time such that no data value is skipped over. The final result from the CNN is then transformed into one long vector by the encoder that is then used by the Neural ODE as the initial state. Subsequent predictions are then made similar to the `ODE` class in which `Neural_ODE` is called to get the state derivatives which are then numberically integrated using the DOPRI-5 method.
